// FROZEN copy of the pre-restructure production prompt builder (commit
// f98ce9a, the state before PR #18), kept ONLY as the "before" arm of the dev
// prefill benchmark (lib/dev/prefill-benchmark.ts). Its defining property is
// the one PR #18 removed: the volatile datetime sits ABOVE the tool-schema
// block, so every clock tick invalidates the KV cache for the whole prompt.
//
// Changes vs f98ce9a: `now` is a parameter instead of new Date() (the
// benchmark steps a deterministic clock), and the function is renamed
// buildSystemPromptV2. Do not "fix" or update the prompt text — a frozen
// baseline that drifts stops being a baseline.

import { formatToolsForPrompt } from "../tools/tool-registry";
import { localDateStr, addDays } from "../orchestrator/date-utils";
